window.addEventListener("DOMContentLoaded", function () {
  $(
    "#product-sale-days .e-tabs .tab-content .products-view-grid .swiper-wrapper"
  ).each(function () {
    var ts = $(this);
    var category = ts.parent().attr("data-label");
    var tab_id = ts.parent().attr("id");
    $.ajax({
      url: "/feeds/posts/default/-/" + category,
      type: "get",
      data: {
        alt: "json",
        "max-results": 8,
      ...
      success: function (k) {
        getData(k);
        awe_lazyloadImage();
        ts.find(".item_add").click(function () {
      ...
      if (void 0 !== entry_post) {
        for (var n = 0; n < e.feed.entry.length; n++) {
          ...
          if ("media$thumbnail" in e.feed.entry[n]) {
            d = e.feed.entry[n].media$thumbnail.url.replace(
              "s72-c",
              "w480-h480-p-k-no-nu"
            );
          } else {
            d =
              "https://3.bp.blogspot.com/-Yw8BIuvwoSQ/VsjkCIMoltI/AAAAAAAAC4c/s55PW6xEKn0/w480-h480-p-k-no-nu/nth.png";
          }
          var thumb = "";
          ...
          if (dataItem) {
            ...
            if (dataItem.oldPrice != null) {
              ...
              var giam = (
                100 *
                ((parseInt(price) - parseInt(old)) / parseInt(old))
              ).toFixed(0);
            ...
            if (dataItem.aff) {
              ...
              linkpost =
                ...
                t +
                '" class="action cart-button"><svg x="0px" y="0px" viewBox="0 0 162.656 162.656" style="enable-background:new 0 0 162.656 162.656;" xml:space="preserve"><path d="M151.764,10.894c-14.522-14.522-38.152-14.525-52.676-0.008l0.003,0.003L76.112,33.872l10.607,10.605l22.983-22.988 l-0.002-0.002c8.678-8.663,22.785-8.658,31.457,0.014c8.673,8.672,8.672,22.786,0,31.461l-34.486,34.484 c-4.201,4.202-9.787,6.516-15.729,6.516c-5.942,0-11.529-2.314-15.73-6.516L64.605,98.052c7.035,7.035,16.389,10.91,26.338,10.91 c9.949,0,19.303-3.875,26.335-10.91l34.487-34.484C166.284,49.043,166.284,25.413,151.764,10.894z"></path><path d="M52.96,141.162L52.96,141.162c-8.675,8.67-22.788,8.668-31.461-0.005c-8.673-8.675-8.673-22.791-0.001-31.465L55.98,75.21 c8.675-8.674,22.789-8.674,31.462,0L98.05,64.604c-14.524-14.523-38.154-14.524-52.676,0L10.89,99.086 c-14.519,14.523-14.519,38.154,0.001,52.678c7.263,7.262,16.801,10.893,26.341,10.892c9.536,0,19.074-3.629,26.333-10.887 l0.002-0.001l22.984-22.99l-10.608-10.606L52.96,141.162z"></path></svg></a>';
            }
            var content =
            '<div class="swiper-slide"> <div class="evo-product-item simpleCart_shelfItem"> <div class="thumb-evo"> ' +
            sale +
            ' <a class="thumb-img" href="' +
            t +
            '" title="' +
            f +
            '"> <span class="tt-img"><span class="d-none item_link">' +
            t +
            '</span><img class="lazy item_thumb" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC" data-src="' +
            d +
            '" alt="' +
            f +
            '" /> </span> ' +
            thumb +
            ' </a> <form class="product-button hidden-sm hidden-xs hidden-md variants form-nut-grid form-ajaxtocart">' +
            buy +
            " " +
            linkpost +
            ' </form> </div> <a href="' +
            ...
            '" title="' +
            f +
            '" class="title item_name">' +
            f +
            '</a> <div class="flex-prices"> <strong class="product-price item_price">' +
            giaban +
            "</strong> " +
            cu +
            " </div> </div> </div>";
          ...
          //         if (
          //           entry.link[c].type != "empty" &&
          //           entry.link[c].type != "image/jpeg" &&
          //           entry.link[c].type != "image/png"
          //         ) {
          //           var giacu = entry.link[c].type;
          //           var cu =
          //             '<span class="product-old-price">' + giacu + "</span>";
          //           var price = giaban.replace(/[^0-9\.]+/gi, "");
          //           var old = giacu.replace(/[^0-9\.]+/gi, "");
          //           var giam = (
          //             100 *
          //             ((parseInt(price) - parseInt(old)) / parseInt(old))
          //           ).toFixed(0);
          ...
          //       }
          //       if (
          //         entry_enclosure != "http://giaban.com" &&
          //         entry_enclosure != "http://giacu.com"
          //       ) {
          //         if (
          //           entry.link[c].type == "image/jpeg" ||
          //           entry.link[c].type == "image/png"
          ...
          //           thumb +=
          //             '<span class="tt-img-roll-over"><img class="lazy" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsQAAA7EAZUrDhsAAAANSURBVBhXYzh8+PB/AAffA0nNPuCLAAAAAElFTkSuQmCC" data-src="' +
          //             entry_enclosure.replace("s16000", "w480-h480-p-k-no-nu") +
          //             '" alt="' +
          //             f +
          //             '" /> </span>';
          //         }
        ...
        var content =
          '<div class="alert alert-warning fade in green-alert show" role="alert"> <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">×</span><span class="sr-only">Close</span></button> Không có sản phẩm nào trong danh mục này. </div>';
        $(
          "#product-sale-days .e-tabs .tab-content .products-view-grid .swiper-wrapper"
        ).each(function () {
          var ths = $(this);
